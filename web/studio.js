...
(() => {
 ...
 function disableExport(on){for(const id of ['download-card','download-svg','download-data','full-preview'])$('#'+id).disabled=on;}
 async function render(){if(!conn)return;const token=++generation;disableExport(true);$('#studio-status').textContent='Querying your selection…';$('#story-card').setAttribute('aria-busy','true');
  try{const s=state();for(const [field,text] of [['player','playerText'],['compare','compareText']]){const name=s[text].trim();if(name){const p=players.find(p=>p.choice===name&&p.gender===s.gender);if(!p)throw new Error('Choose a player from the suggestions for the selected gender.');s[field]=p.player_id;}}
   ...
   $('#sql').textContent=sql;const result=await query(sql);if(token!==generation)return;rows=result;current=s;page=0;
 ...
 function paint(){if(!current||!rows.length){$('#story-card').innerHTML='<p class="studio-empty">No matching records. Try widening your selection.</p>';$('#studio-result-table').textContent='';return;}
  ...
  const data=dataView(),perPage=design.size==='portrait'?24:design.size==='square'?15:8,pages=Math.ceil(data.length/perPage);page=Math.max(0,Math.min(page,pages-1));const subset=data.slice(page*perPage,(page+1)*perPage);
  const metric=core.metrics[current.metric],baseTitle=current.playerText?current.playerText.split(' · ')[0]:current.team||current.venue||metric+' in international cricket';const title=design.title||baseTitle;
  ...
  let top=116;for(const l of wrap(title,48).slice(0,3)){svg+=text(48,top,l,34);top+=40;}if(design.subtitle){for(const l of wrap(design.subtitle,78).slice(0,3)){svg+=text(48,top,l,19,muted);top+=25;}}
  ...
  svg+=text(48,H-25,'cricket.rkjat.in · Cricsheet',16,muted)+text(W-48,H-25,(current.dataset==='careers'?'Career check '+String(manifest.career_checked_at||'Unknown').slice(0,10):'Archive through '+manifest.match_date_to),15,muted,'end')+'</svg>';
 ...
 function bind(){document.querySelectorAll('[data-template]').forEach(b=>b.onclick=()=>preset(b.dataset.template));$('#studio-form').onsubmit=e=>{e.preventDefault();render();};$('#studio-dataset').onchange=fields;$('#studio-format').onchange=fields;$('#studio-gender').onchange=()=>{fields();$('#player-one').value='';$('#player-two').value='';};
  ...
  $('#save-design').onclick=()=>{try{localStorage.setItem('cw-studio-design',JSON.stringify(state()));$('#studio-status').textContent='Design saved on this device.';}catch{$('#studio-status').textContent='This browser could not save the design.';}};
  $('#load-design').onclick=()=>{try{const saved=JSON.parse(localStorage.getItem('cw-studio-design'));if(!saved)throw new Error();restore(saved);render();}catch{$('#studio-status').textContent='No valid saved design on this device.';}};
  $('#reset-design').onclick=()=>{for(const [k,v] of Object.entries({type:'bar',size:'landscape',theme:'light',accent:'#1d4ed8',title:'',subtitle:'',labels:'on'}))$('#'+ids[k]).value=v;paint();};
 }
