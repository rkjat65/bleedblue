...
def write_table(name,rows):
    ...
    return {'file':versioned.name,'rows':len(rows),'bytes':target.stat().st_size,'sha256':hashlib.sha256(target.read_bytes()).hexdigest(),'columns':[{'name':f.name,'type':str(f.type)} for f in table.schema]}
