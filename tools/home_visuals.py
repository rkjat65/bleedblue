...
def homepage_insights(matches, match_routes, people, team_routes):
    ...
    return f'''<section class="home-insights" aria-labelledby="home-insights-title"><div class="section-heading"><div><p class="eyebrow">THE CRICKET WICKET VIEW</p><h2 id="home-insights-title">Start with the questions that define cricket.</h2><p class="muted">Use the numbers to compare teams, players and the tournaments that shaped the international game.</p></div><a href="/studio/">Build your own chart →</a></div>
      ...
      <article class="home-insight-card home-world"><div class="home-card-label"><span>WORLD CUP ARCHIVE · MEN &amp; WOMEN</span><strong>{world_total:,}</strong><small>recorded matches</small></div><h3>Every World Cup era in one place.</h3><p>Open winners, run scorers, wicket takers and scorecards for the men’s and women’s global game.</p><figure class="home-chart home-world-chart" aria-label="World Cup matches by tournament category"><figcaption>Recorded matches by tournament category</figcaption>{world_chart}</figure><div class="table-wrap"><table><caption>World Cup matches in the published archive</caption><thead><tr><th>Tournament</th><th>Matches</th></tr></thead><tbody>{world_rows or '<tr><td colspan="2">No tournament label recorded</td></tr>'}</tbody></table></div><a class="home-card-link" href="/world-cup/">World Cup winners and leaders →</a></article></div>
