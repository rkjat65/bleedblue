...
def main():
    ...
    (OUT/'sitemap.xml').write_text('<?xml version="1.0" encoding="UTF-8"?><sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+''.join(f'<sitemap><loc>{BASE}/{name}</loc><lastmod>{lastmod}</lastmod></sitemap>' for name,lastmod in sitemap_files)+'</sitemapindex>',encoding='utf-8')
