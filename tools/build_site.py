...
def player_illustration(p):
    path=ILLUSTRATIONS.get(p.get('name'))
    if not path:return ''
    return '<aside class="player-illustration"><img src="'+path+'" width="420" height="480" loading="lazy" alt="Editorial caricature of '+esc(p['name'])+' holding a cricket bat"><div><span class="eyebrow">CRICKET WICKET ILLUSTRATION</span><h2>'+esc(p['name'])+'</h2><p>A Cricket Wicket editorial caricature, shown alongside the verified career record.</p></div></aside>'
...
def main():
    ...
    for pid,p in people.items():
        ...
        body=profile_title+player_illustration(p)+profile_coverage(p,rows)+actions()+'<div class="stats">'+''.join(f'<div><strong>{num(tot.get(k))}</strong><span>{label}</span></div>' for k,label in [('matches','Internationals'),('runs','Career runs'),('hundreds','Centuries'),('wickets','Wickets')])+'</div>'
...
def build_daily_blog(people,pp,careers):
    ...
    posts=[]
    def publish(slug_name,title,description,visual,alt,lead,content,callout,width=1432,height=1076):
        path='/blog/'+post_date+'/'+((slug_name.strip('/')+'/') if slug_name else '')
        article='<div class="daily-note">'+heading(title,'A small daily lesson in reading international cricket numbers.','DAILY NOTE')
        article+=f'<p class="daily-byline"><span>Cricket Wicket</span><span>·</span><time datetime="{post_date}">{post_date}</time><span>·</span><span>Data checked {checked}</span></p>'+actions()
        article+='<article><p>'+lead+'</p><figure class="daily-infographic"><img src="/assets/art/blog/'+visual+'" width="'+str(width)+'" height="'+str(height)+'" loading="eager" alt="'+esc(alt)+'"><figcaption>'+esc(title)+' · Cricket Wicket · data checked '+checked+'</figcaption></figure>'+content
        article+='<div class="data-callout">'+callout+'</div><p>'+a('/compare/','Compare two international careers')+' · '+a(pp[left['id']],left['name']+' profile')+' · '+a(pp[right['id']],right['name']+' profile')+'</p><p class="note">Career figures are independent official snapshots. Batting average is runs ÷ dismissals; strike rate is 100 × runs ÷ balls faced. A missing denominator stays unavailable.</p></article></div>'
        ...
        posts.append((path,title,description))
    ...
    index=heading('Cricket Wicket daily notes','Small, useful lessons from international cricket records, written with the data in view.','DAILY NOTES')+actions()+'<div class="grid three daily-index-grid">'+''.join('<a class="feature-card daily-index-card" href="'+path+'"><span>10 SEPTEMBER 2026</span><h2>'+esc(title)+'</h2><p>'+esc(description)+'</p><small>Read this note →</small></a>' for path,title,description in posts)+'</div><section class="panel"><h2>Keep exploring</h2><p>'+a('/questions/','Cricket questions answered with data')+' · '+a('/records/','International records')+' · '+a('/insights/','Statistical insights')+'</p></section>'
...
def build_collections(people,matches,pp,mp,gp,groups,careers,arc,hist,editorial=None,all_cards=None):
    ...
    for left,right in [('Virat Kohli','Rohit Sharma'),('Sachin Tendulkar','Don Bradman'),('Joe Root','Steve Smith'),('Mithali Raj','Meg Lanning'),('Jasprit Bumrah','James Anderson')]:
        ...
        if left in ILLUSTRATIONS and right in ILLUSTRATIONS:
            pair_art='<div class="comparison-illustrations"><div><img src="'+ILLUSTRATIONS[left]+'" width="420" height="480" alt="Editorial caricature of '+esc(left)+'"><span>'+esc(left)+'</span></div><div class="comparison-illustrations-copy"><span class="eyebrow">FEATURED COMPARISON</span><p>Explore the numbers side by side, with the same format and career scope.</p></div><div><img src="'+ILLUSTRATIONS[right]+'" width="420" height="480" alt="Editorial caricature of '+esc(right)+'"><span>'+esc(right)+'</span></div></div>'
        content=heading(left+' vs '+right,'Career comparison by format. Different eras and sample sizes need context.','PLAYER COMPARISON')+pair_art+actions()
        for fmt in ['Test','ODI','T20I']:
