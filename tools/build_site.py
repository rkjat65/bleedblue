...
def build_daily_blog(people,pp,careers):
    """Publish one concise, data-backed daily note with a shareable graphic."""
    ...
    title='How to compare two international batters fairly'
    description='A practical Cricket Wicket guide to reading runs, batting average and strike rate together, using a Virat Kohli and Rohit Sharma ODI example.'
    article='<div class="daily-note">'+heading(title,'A small daily lesson in reading international cricket numbers.','DAILY NOTE')
    article+=f'<p class="daily-byline"><span>Cricket Wicket</span><span>·</span><time datetime="{post_date}">{post_date}</time><span>·</span><span>Data checked {esc(careers["meta"].get("checked_at",TODAY)[:10])}</span></p>'+actions()
    article+='<article><p>When two batters are compared, one number rarely settles the question. Runs show accumulated output, average shows what a player scores per dismissal, and strike rate shows scoring pace. Read the three together, and keep the format and career scope the same.</p>'
    article+='<figure class="daily-infographic"><img src="/assets/art/blog/2026-09-10-compare-batters.svg" width="1200" height="760" loading="eager" alt="Infographic comparing Virat Kohli and Rohit Sharma in ODI runs, batting average and strike rate"><figcaption>Virat Kohli and Rohit Sharma ODI career snapshot · Cricket Wicket · data checked '+esc(careers['meta'].get('checked_at',TODAY)[:10])+'</figcaption></figure>'
    article+='<h2>What the example shows</h2><p>In this ODI snapshot, '+a(pp[left['id']],left['name'])+' leads the run total with <strong>'+num(a1.get('runs'))+'</strong> from '+num(a1.get('innings'))+' innings. '+a(pp[right['id']],right['name'])+' has '+num(a2.get('runs'))+' runs from '+num(a2.get('innings'))+' innings. That is the volume view.</p>'
    article+='<div class="data-callout"><div><strong>'+decimal_stat(a1.get('avg'))+'</strong><span>'+left['name']+' ODI average</span></div><div><strong>'+decimal_stat(a2.get('avg'))+'</strong><span>'+right['name']+' ODI average</span></div><div><strong>'+decimal_stat(a1.get('sr'))+' / '+decimal_stat(a2.get('sr'))+'</strong><span>ODI strike rates · '+left['name']+' / '+right['name']+'</span></div></div>'
    article+='<p>The average comparison favours '+left['name']+' in this snapshot, while the strike-rate values are close. Neither result is a complete judgement: era, role, opposition, venues and opportunity still matter. A number is most useful when its denominator and context are visible.</p>'
    article+='<h2>A three-step habit</h2><ol><li>Choose the same format and gender.</li><li>Read runs or wickets beside innings, dismissals and balls.</li><li>Use the comparison tool and profile explorer to inspect the sample before drawing a conclusion.</li></ol>'
    article+='<p>'+a('/compare/','Compare two international careers')+' · '+a(pp[left['id']],left['name']+' profile')+' · '+a(pp[right['id']],right['name']+' profile')+'</p><p class="note">Career figures are independent official snapshots. Strike rate is 100 × runs ÷ balls faced; batting average is runs ÷ dismissals. A missing denominator stays unavailable.</p></article></div>'
    extra={'headline':title,'datePublished':post_date,'dateModified':post_date,'author':{'@type':'Organization','name':'Cricket Wicket','url':BASE+'/about/'},'publisher':{'@type':'Organization','name':'Cricket Wicket','url':BASE+'/'}}
    page('/blog/'+post_date+'/',title,description,article,'Article',extra)
    index=heading('Cricket Wicket daily notes','Small, useful lessons from international cricket records, written with the data in view.','DAILY NOTES')+actions()+'<div class="grid three daily-index-grid"><a class="feature-card daily-index-card" href="/blog/'+post_date+'/" ><span>10 SEPTEMBER 2026</span><h2>'+esc(title)+'</h2><p>'+esc(description)+'</p><small>Read today’s note →</small></a></div><section class="panel"><h2>Keep exploring</h2><p>'+a('/questions/','Cricket questions answered with data')+' · '+a('/records/','International records')+' · '+a('/insights/','Statistical insights')+'</p></section>'
