...
def main():
    ...
    for path in sorted((ROOT/'analytics_lake').iterdir()):
        if not path.is_file():continue
        content_type='application/json' if path.suffix=='.json' else 'application/vnd.apache.parquet'
        client.upload_file(str(path),BUCKET,path.name,ExtraArgs={'ContentType':content_type,'CacheControl':'public, max-age=3600, stale-while-revalidate=86400'})
